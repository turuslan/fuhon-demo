...
import { delay } from "https://deno.land/std@0.135.0/async/delay.ts";
...
function repoInfo(repo: string) {
  let match: RegExpMatchArray;
  let token: string;
  try {
    match = Deno.readTextFileSync(join(repo, "api")).match(
      /\/ip4\/([^/]+)\/tcp\/([^/]+)(\/http|$)/,
    )!;
  ...
  const host = match[1];
  const port = +match[2];
  return `ws://${host}:${port}/rpc/v0?token=${token}`;
}
